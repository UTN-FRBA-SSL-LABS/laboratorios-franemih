...
> R: carrito_init limpia los valores residuales del carrito, lo cual evita errores en los calculos tras los usos (si se usa una vez va a funcionar pero el resultado va a arrastrar un error en los siguientes usos).
...
> R:El test falla porque obtuvo 350 mientras que esperaba 700
...
> R: El bug esta en la linea 24(linea completa:total += c->items[i].precio;  /* BUG: falta multiplicar por cantidad */)

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Deberia multiplicar el valor con la cantidad buscada para obtener el resultado esperado
...
> R:El test fallo -  [FAIL] carrito_agregar(&c, p5) => esperado 0, obtenido 1  (linea 59)
...
> R: El operador [<=] permite un quinto elemento (el array va de 0 a 4), entonces deberia ser [<]
...
> R: Mientras que el test unitario funciona para probar de forma aislada las funciones, el test de integracion genera un flujo de prueba (adaptado al ideal del programa) analizando la sinergia de varias funciones a la vez, el que detecto primero el primer bug fue el test unitario.
...
> R: Mientras que java y python son de mas alto nivel y logran utilizar de forma mas generalizada los arrays (el mismo lenguaje detecta el limite necesario), en C se debe definir todo de forma manual, entonces mientras que C confia en lo que escribe el programador (no verifica elementos como esos) puede generar ese tipo de errores, los lenguajes como Python y Java verifican los limites y genera Warnings a la hora de copilar, evitando este tipo de bugs.
...
> R:Es mejor porque ayuda a reducir el codigo y saber precisamente en que falla aunque en el codigo no se vea a simple vista.
...
> R: Matematicamente hablando el primer caso tenia cantidad = 1, y en la multiplicacion para todo x: x*1=x, por eso en el segundo caso (con cantidad=2) fallo pero el primero paso.
